...
def convert_programme_courses(df):
    for _, row in df.iterrows():
        pc_uri = uri("ProgrammeCourse", f"{row['programmeCode']}_{row['courseCode']}")
...
def convert_registrations(df):
    for _, row in df.iterrows():
        reg_uri = uri("Registration", row['registrationId'])
...
def convert_course_instances(df):
    for _, row in df.iterrows():
        ...
        if pd.notna(row.get('academicYear')):
            g.add((ci_uri, EX.academicYear, Literal(row['academicYear'], datatype=XSD.string)))
        if pd.notna(row.get('studyPeriod')):
            g.add((ci_uri, EX.studyPeriod, Literal(int(row['studyPeriod']), datatype=XSD.integer)))
        if pd.notna(row.get('planningNumStudents')):
            g.add((ci_uri, EX.planningNumStudents, Literal(int(row['planningNumStudents']), datatype=XSD.integer)))
        if pd.notna(row.get('Examiner')):
            g.add((ci_uri, EX.examinedBy, uri("Teacher", row['Examiner'])))
...
convert_course_instances(instances_df)
